...
def test_export_project_video_with_ia(tmp_path):
    """
    Testa pipeline IA completo: gera TTS, avatar e exporta vídeo final MP4 com MoviePy.
    """
    from app.models import Project, Scene, Asset
    from app.services.video_pipeline_service import export_project_video_with_ia
    # Projeto e cenas de exemplo
    project = Project(id=999, name="Projeto Teste IA")
    scenes = [
        Scene(id=1, project_id=999, nome="Cena 1", ordem=0, texto="Olá, este é um teste de narração IA.", duracao=3.0),
        Scene(id=2, project_id=999, nome="Cena 2", ordem=1, texto="Esta é a segunda cena do vídeo.", duracao=3.0)
    ]
    assets = [
        Asset(id=1, scene_id=1, project_id=999, tipo="image", camada=0, caminho_arquivo="app/static/backgrounds/default_bg.png"),
        Asset(id=2, scene_id=2, project_id=999, tipo="image", camada=0, caminho_arquivo="app/static/backgrounds/default_bg.png")
    ]
    # Executar pipeline IA
    final_video_path = export_project_video_with_ia(project, scenes, assets, tts_model="coqui", avatar_model="hunyuan3d2")
    assert os.path.exists(final_video_path)
    assert final_video_path.endswith(".mp4")

def test_export_project_video_with_ia_invalid_tts(tmp_path):
    """
    Testa pipeline IA com modelo TTS inválido e espera erro controlado.
    """
    from app.models import Project, Scene, Asset
    from app.services.video_pipeline_service import export_project_video_with_ia
    import pytest
    project = Project(id=998, name="Projeto Teste IA Erro")
    scenes = [Scene(id=1, project_id=998, nome="Cena 1", ordem=0, texto="Teste erro TTS", duracao=3.0)]
    assets = [Asset(id=1, scene_id=1, project_id=998, tipo="image", camada=0, caminho_arquivo="app/static/backgrounds/default_bg.png")]
    with pytest.raises(ValueError):
        export_project_video_with_ia(project, scenes, assets, tts_model="modelo_invalido", avatar_model="hunyuan3d2")

def test_export_ia_status(client, db_session):
    """
    Testa endpoint de status de exportação IA.
    """
    from app.models import Video
    # Simula vídeo em processamento
    video = Video(id=1000, project_id=42, title="Teste", filename="video.mp4", file_path="/static/videos/generated/project_42/final_project_video.mp4", file_size=1234, duration=10, format="mp4", status="processing")
    db_session.add(video)
    db_session.commit()
    response = client.get("/api/export-ia-status/42")
    assert response.status_code == 200
    assert response.json()["status"] == "processing"
    # Simula vídeo concluído
...
def test_import_presentations(client, db_session, tmp_path):
    """
    Testa upload e importação de PDF e PPTX, verifica criação de cenas e preview.
    """
    from app.models import Project
    # Cria projeto de teste
    project = Project(id=77, name="Projeto Importação Teste")
    db_session.add(project)
    db_session.commit()
    # Cria arquivos de exemplo
    pdf_path = tmp_path / "exemplo.pdf"
    pptx_path = tmp_path / "exemplo.pptx"
    with open(pdf_path, "wb") as f:
        f.write(b"%PDF-1.4\n1 0 obj\n<< /Type /Catalog >>\nendobj\ntrailer\n<<>>\nstartxref\n0\n%%EOF")
    with open(pptx_path, "wb") as f:
        f.write(b"PK\x03\x04\x14\x00\x06\x00")  # Header mínimo PPTX
    with open(pdf_path, "rb") as pdf_file, open(pptx_path, "rb") as pptx_file:
        response = client.post(
            "/api/import-presentations/",
            data={"project_id": 77},
            files=[
                ("files", ("exemplo.pdf", pdf_file, "application/pdf")),
                ("files", ("exemplo.pptx", pptx_file, "application/vnd.openxmlformats-officedocument.presentationml.presentation")),
            ],
        )
    assert response.status_code == 200
    data = response.json()
    assert data["success"] is True
    assert isinstance(data["created_scenes"], list)

def test_list_supported_import_formats(client):
    """
    Testa endpoint de listagem de formatos suportados para importação automática.
    """
    response = client.get("/api/import-supported-formats/")
    assert response.status_code == 200
    data = response.json()
