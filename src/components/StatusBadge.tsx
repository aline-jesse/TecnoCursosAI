...
import { ProjectStatus, StatusBadgeProps } from '../types/project'
...
export const StatusBadgeWithProgress: React.FC<StatusBadgeWithProgressProps> = ({
  ...
  const isProcessing = status === ProjectStatus.PROCESSING || status === ProjectStatus.RENDERING
...
export const StatusSummary: React.FC<StatusSummaryProps> = ({
  ...
  const nonZeroCounts = Object.entries(counts).filter(([_, count]) => count && count > 0)
...
export const isStatusInProgress = (status: ProjectStatus): boolean => {
  return status === ProjectStatus.PROCESSING || status === ProjectStatus.RENDERING
}
