...
const TimelineControls: React.FC<TimelineControlsProps> = ({
   ...
   */
  const getTimelinePosition = useCallback((time: number): number => {
  ...
  }, [newMarkerTime, newMarkerLabel, onMarkerAdd])
   ...
   */
  const renderWaveform = useCallback((canvas: HTMLCanvasElement, waveformData: number[], width: number, height: number) => {
  ...
  useEffect(() => {
    if (config.audio && config.audio.waveformData && waveformCanvasRef.current) {
      renderWaveform(waveformCanvasRef.current, config.audio.waveformData, 400, 40)
    }
    ...
    <div className="timeline-controls">
      ...
      <div className="timeline-header">
        ...
        <div className="playback-speed">
          ...
          >
            {config.playbackSpeed}x <Cog6ToothIcon style={{ width: '1rem', height: '1rem' }} />
          </button>
      ...
      <div className="timeline-container">
        ...
        <div className="timeline-scale">
          {Array.from({ length: Math.ceil(config.duration / 10) + 1 }, (_, i) => {
            const time = i * 10
      ...
      {config.audio && config.audio.waveformData && (
        <div className="waveform-container">
          <canvas ref={waveformCanvasRef} width={400} height={40} style={{ width: '100%', height: 40 }} />
        </div>
      )}
      ...
      <div className="scene-navigation">
        ...
        <div className="scene-buttons">
          <button
            onClick={() => onSceneSelect(Math.max(0, playerState.currentSceneIndex - 1))}
          ...
          <span className="scene-indicator">
            Cena {playerState.currentSceneIndex + 1}
          </span>
          
          <button
            onClick={() => onSceneSelect(playerState.currentSceneIndex + 1)}
            className="nav-button"
          >
            Próxima
        ...
        <div className="marker-dialog-overlay">
          <div className="marker-dialog">
            ...
            <div className="dialog-content">
              <label>
                Tempo: {formatTime(newMarkerTime)}
              </label>
              <label>
                Nome do Marcador:
                <input
