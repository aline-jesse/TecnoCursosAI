...
const MockEditorCanvas = ({ activeScene, onUpdateScene, assets }) => {
  ...
  const handleDeleteSelected = () => {
    if (selectedElement) {
      const newScene = {
        ...scene,
        objects: (scene.objects || []).filter(obj => obj.id !== selectedElement)
      };
      setScene(newScene);
...
describe('EditorCanvas - Funcionalidades Básicas', () => {
  ...
  describe('Renderização', () => {
    ...
    test('deve mostrar informações da cena', () => {
      ...
      expect(screen.getByTestId('objects-count')).toHaveTextContent('Objetos: 0');
      expect(screen.getByTestId('selected-element')).toHaveTextContent('Selecionado: Nenhum');
    });
  });
  ...
  describe('Funcionalidades de Controle', () => {
    test('deve adicionar texto quando botão "Adicionar Texto" for clicado', async () => {
      ...
      expect(screen.getByTestId('objects-count')).toHaveTextContent('Objetos: 1');
    });
    ...
    test('deve deletar elemento selecionado', async () => {
      ...
      const sceneWithObject = {
        ...mockScene,
        objects: [
          { id: 'obj1', type: 'text', text: 'Texto para deletar', x: 100, y: 100, width: 200, height: 40 }
        ],
      };
  ...
  describe('Estados e Props', () => {
    test('deve atualizar quando props mudarem', () => {
      ...
      const newScene = {
        ...
        name: 'Nova Cena',
        objects: [
          { id: 'obj1', type: 'text', text: 'Novo texto', x: 50, y: 50, width: 100, height: 30 }
        ],
      };
  ...
  describe('Interações de Elementos', () => {
    test('deve selecionar elemento quando clicado', () => {
      const sceneWithObject = {
        ...mockScene,
        objects: [
          { id: 'obj1', type: 'text', text: 'Texto teste', x: 100, y: 100, width: 200, height: 40 }
        ],
      };
      ...
      expect(screen.getByTestId('selected-element')).toHaveTextContent('Selecionado: obj1');
    });
    ...
    test('deve mostrar objetos da cena', () => {
      const sceneWithObjects = {
        ...mockScene,
        objects: [
          { id: 'obj1', type: 'text', text: 'Texto 1', x: 100, y: 100, width: 200, height: 40 },
          { id: 'obj2', type: 'image', assetId: 'img1', x: 200, y: 200, width: 150, height: 100 }
        ],
      };
      
      renderEditorCanvas({ activeScene: sceneWithObjects });
      
      expect(screen.getByTestId('object-obj1')).toHaveTextContent('text: Texto 1');
      expect(screen.getByTestId('object-obj2')).toHaveTextContent('image: img1');
    });
  });
}); 