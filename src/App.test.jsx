...
function testAppComponent() {
  ...
  function testLayoutStructure() {
    ...
    try {
      ...
      const hasToolbar = children.some(child => child.props.className === 'toolbar');
      const hasEditorMain = children.some(child => child.props.className === 'editor-main');
      const hasTimeline = children.some(child => child.props.className === 'timeline');
  ...
  function testIntegratedComponents() {
    ...
    try {
      const result = App();
      const editorMain = result.props.children.find(child => child.props.className === 'editor-main');
      const children = editorMain.props.children;
      
      // Verifica se tem asset-panel, editor-canvas e scene-list
      const hasAssetPanel = children.some(child => child.props.className === 'asset-panel');
      const hasEditorCanvas = children.some(child => child.props.className === 'editor-canvas');
      const hasSceneList = children.some(child => child.props.className === 'scene-list');
      
      if (hasAssetPanel && hasEditorCanvas && hasSceneList) {
        console.log('✅ Teste 4: Componentes integrados corretamente - PASSOU');
        testsPassed++;
      } else {
        console.log('❌ Teste 4: Componentes não integrados corretamente - FALHOU');
      }
    } catch (error) {
