...
const useSceneList = projectId => {
  ...
  const addScene = useCallback(async () => {
    ...
    try {
      ...
      const newSceneData = sceneListService.createDefaultSceneData(
        scenes.length + 1
      );
      ...
      const newScene = await sceneListService.createScene(
        projectId,
        newSceneData
      );
  ...
  const removeScene = useCallback(
    async sceneId => {
      ...
      if (scenes.length <= 1) {
        handleError(
          new Error('Não é possível remover a única cena'),
          'Remoção de cena'
        );
        return;
      }
  ...
  const duplicateScene = useCallback(
    async sceneId => {
      ...
      try {
        ...
        const duplicatedScene = await sceneListService.duplicateScene(
          projectId,
          sceneId
        );
  ...
  const reorderScenes = useCallback(
    async (sourceIndex, destinationIndex) => {
      if (!projectId) {
        handleError(
          new Error('Projeto não selecionado'),
  ...
  const updateScene = useCallback(
    async (sceneId, updates) => {
      ...
      try {
        ...
        const updatedScene = await sceneListService.updateScene(
          projectId,
          sceneId,
          updates
        );
        ...
        setScenes(prevScenes =>
          prevScenes.map(scene => (scene.id === sceneId ? updatedScene : scene))
        );
  ...
  const getScenesStats = useCallback(() => {
    ...
    const totalAssets = scenes.reduce(
      (total, scene) => total + (scene.assets?.length || 0),
      0
    );
