...
const PreviewCanvas: React.FC<PreviewCanvasProps> = ({
  ...
  useEffect(() => {
    ...
    if (!context) {
      onError?.('Não foi possível obter contexto 2D do canvas');
      return;
    }
  ...
  const loadVideo = useCallback(
    async (src: string): Promise<HTMLVideoElement> => {
      // Verifica se já está no cache
    ...
    (
      ...
      if (timeSinceStart >= 0 && timeSinceStart <= animationDuration) {
        ...
        switch (animation.type) {
          ...
          case 'bounce':
            const bounceValue =
              Math.sin(progress * Math.PI * 4) * (1 - progress) * 10;
    ...
    <div className='preview-canvas-wrapper'>
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className='preview-debug-info'>
          ...
          <div>
            Resolução: {qualitySettings.width}x{qualitySettings.height}
          </div>
