...
const VideoEditorIntegration = () => {
  ...
  const handleFileUpload = async (file, projectId = null) => {
    try {
      ...
      if (!allowedTypes.includes(`.${fileExtension}`)) {
        throw new Error(
          'Tipo de arquivo não permitido. Use PDF, PPTX ou DOCX.'
        );
      }
  ...
  const generateVideo = async (options = {}) => {
    try {
      ...
      // Gerar vídeo
      const generationTask = await videoService.generateVideo(
        currentProject.id,
        {
          resolution: '1920x1080',
          quality: 'high',
      ...
      const checkProgress = async () => {
        try {
          ...
          if (status.status === 'completed') {
            ...
            // Download do vídeo
            const videoBlob = await videoService.downloadVideo(
              currentProject.id
            );
      ...
      <div className='scene-editor'>
        ...
        <div className='form-group'>
          ...
          <input
            ...
            onChange={e =>
              setSceneData({ ...sceneData, title: e.target.value })
            }
        ...
        <div className='form-group'>
          ...
          <textarea
            ...
            onChange={e =>
              setSceneData({ ...sceneData, content: e.target.value })
            }
            className='form-control'
            rows='4'
          />
        ...
        <div className='form-group'>
          ...
          <input
            ...
            onChange={e =>
              setSceneData({ ...sceneData, duration: e.target.value })
            }
            className='form-control'
            min='1000'
            max='300000'
        ...
        <div className='form-group'>
          ...
          <select
            ...
            onChange={e =>
              setSceneData({ ...sceneData, background: e.target.value })
            }
            className='form-control'
          >
        ...
        <div className='form-group'>
          ...
          <select
            ...
            onChange={e =>
              setSceneData({ ...sceneData, character: e.target.value })
            }
            className='form-control'
          >
  ...
  const VideoGenerationComponent = () => {
    return (
      <div className='video-generation-container'>
        ...
        {!currentProject && <p>Selecione um projeto para gerar vídeo</p>}
      </div>
    );
  };
      ...
      <div className='system-status'>
        ...
        {systemHealth ? (
          <div className='status-info'>
            <p>
              Status: <span className='status-healthy'>✅ Saudável</span>
            </p>
            <p>
              Uptime: {Math.floor(systemHealth.uptime / 3600)}h{' '}
              {(systemHealth.uptime % 3600) / 60}m
            </p>
            <p>Versão: {systemHealth.version}</p>
          </div>
        ) : (
          <p>
            Status: <span className='status-error'>❌ Indisponível</span>
          </p>
        )}
    ...
    <div className='video-editor-integration'>
      ...
      {currentProject && (
        <div className='current-project-info'>
          ...
          {currentScene && <p>Cena Selecionada: {currentScene.title}</p>}
        </div>
      )}
    </div>
  );
