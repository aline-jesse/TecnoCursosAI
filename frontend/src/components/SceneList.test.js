...
const mockScenes = [
  {
    ...
    assets: [{ id: 'asset-1', thumbnail_url: 'http://example.com/thumb1.jpg' }],
...
const renderWithDragDrop = component => {
  return render(
    <DragDropContext onDragEnd={jest.fn()}>{component}</DragDropContext>
  );
};
...
describe('SceneList', () => {
  ...
  describe('Estados de carregamento e erro', () => {
    ...
    test('mostra estado vazio quando não há cenas', () => {
      ...
      expect(
        screen.getByText('Clique no botão + para adicionar uma cena')
      ).toBeInTheDocument();
    });
  });
  ...
  describe('Interações do usuário', () => {
    test('chama onSceneSelect quando uma cena é clicada', () => {
      ...
      renderWithDragDrop(
        <SceneList {...defaultProps} onSceneSelect={onSceneSelect} />
      );
    ...
    test('chama onSceneAdd quando botão de adicionar é clicado', () => {
      ...
      renderWithDragDrop(
        <SceneList {...defaultProps} onSceneAdd={onSceneAdd} />
      );
    ...
    test('chama onSceneRemove quando botão de remover é clicado', () => {
      ...
      renderWithDragDrop(
        <SceneList {...defaultProps} onSceneRemove={onSceneRemove} />
      );
    ...
    test('chama onSceneDuplicate quando botão de duplicar é clicado', () => {
      ...
      renderWithDragDrop(
        <SceneList {...defaultProps} onSceneDuplicate={onSceneDuplicate} />
      );
  ...
  describe('Formulário de criação de cena', () => {
    test('mostra formulário quando botão de adicionar é clicado', () => {
      ...
      expect(
        screen.getByPlaceholderText('Nome da nova cena')
      ).toBeInTheDocument();
    ...
    test('cria nova cena quando formulário é submetido', () => {
      ...
      expect(onSceneAdd).toHaveBeenCalledWith(
    ...
    test('cancela criação quando botão cancelar é clicado', () => {
      ...
      expect(
        screen.queryByPlaceholderText('Nome da nova cena')
  ...
  describe('Drag and Drop', () => {
    ...
    test('chama onSceneReorder quando drag-and-drop é executado', () => {
      ...
      renderWithDragDrop(
        <SceneList {...defaultProps} onSceneReorder={onSceneReorder} />
      );
  ...
  describe('Renderização de miniaturas', () => {
    ...
    test('renderiza ícone de texto quando cena tem texto', () => {
      ...
      renderWithDragDrop(
        <SceneList {...defaultProps} scenes={scenesWithText} />
      );
  ...
  describe('Formatação de duração', () => {
    ...
    test('mostra 00:00 para duração zero ou negativa', () => {
      ...
      renderWithDragDrop(
        <SceneList {...defaultProps} scenes={scenesWithZeroDuration} />
      );
    ...
    test('formata duração longa corretamente', () => {
      ...
      renderWithDragDrop(
        <SceneList {...defaultProps} scenes={scenesWithLongDuration} />
      );
  ...
  describe('Acessibilidade', () => {
    test('botões têm títulos descritivos', () => {
      ...
      const duplicateButton =
        screen.getByTestId('duplicate-icon').parentElement;
      const removeButton = screen.getByTestId('trash-icon').parentElement;
  ...
  describe('Performance', () => {
    test('renderiza lista grande sem problemas', () => {
      ...
      renderWithDragDrop(
        <SceneList {...defaultProps} scenes={largeScenesList} />
      );
