...
jest.mock('@hello-pangea/dnd', () => ({
  DragDropContext: ({ children, onDragEnd }) => (
    <div
      ...
      onClick={() => onDragEnd({ destination: { index: 1 } })}
    >
      {children}
    </div>
  ),
...
describe('SceneList', () => {
  ...
  describe('Interações do usuário', () => {
    test('permite selecionar uma cena', () => {
      ...
      const sceneItem = screen
        .getByText('Conceitos Básicos')
        .closest('.scene-item');
    ...
    test('permite remover uma cena', async () => {
      ...
      expect(mockHandlers.onSceneRemove).toHaveBeenCalledWith(
        'scene-2',
        expect.any(Object)
      );
    });
    ...
    test('permite duplicar uma cena', async () => {
      ...
      expect(mockHandlers.onSceneDuplicate).toHaveBeenCalledWith(
        'scene-1',
        expect.any(Object)
      );
    });
  ...
  describe('Formatação e utilitários', () => {
    ...
    test('exibe texto da cena truncado', () => {
      ...
      expect(
        screen.getByText('Vamos aprender os fundamentos...')
      ).toBeInTheDocument();
