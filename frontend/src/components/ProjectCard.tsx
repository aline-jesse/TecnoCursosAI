 ...
 */
const PROJECT_TYPE_ICONS: { [key in ProjectType]: React.ElementType } = {
  [ProjectType.VIDEO]: FilmIcon,
  [ProjectType.PRESENTATION]: PhotoIcon,
  [ProjectType.ANIMATION]: PlayIcon,
  [ProjectType.SOCIAL]: ShareIcon, // Adicionado para cobrir todos os tipos
...
export const ProjectCard: React.FC<ProjectCardProps> = ({
  ...
  const canDownload =
    project.status === ProjectStatus.COMPLETED && project.downloadUrl;
  ...
  return (
    ...
    >
      ...
      <div className={`p-4 ${compactMode ? 'py-2' : ''}`}>
        ...
        <div className='mb-2'>
          <h3
            className={`font-semibold text-gray-900 line-clamp-2 ${compactMode ? 'text-sm' : 'text-base'}`}
          >
            {project.name}
          </h3>
        ...
        {!compactMode && (
          <div className='space-y-2 mb-3'>
            ...
            <div className='flex items-center justify-between text-sm text-gray-600'>
              ...
              {project.renderStats?.durationSeconds && (
                <div className='flex items-center gap-1'>
                  <ClockIcon className='w-4 h-4' />
                  <span>
                    {formatDuration(project.renderStats.durationSeconds)}
                  </span>
                </div>
              )}
            </div>
        ...
        <div className='flex items-center justify-between'>
          ...
          <span
            ...
            title={format(project.updatedAt, 'PPpp', { locale: ptBR })}
          >
      ...
      <div className='absolute inset-0 z-0' onClick={handleView} />
    </div>
  );
};
